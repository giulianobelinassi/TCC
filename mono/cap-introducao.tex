...
CUDA execution is divided into a host (CPU) and device (GPU) parts. For the device part, CUDA uses the concept of
...
at the launch moment, thus it can be adjusted to match the problem input size.


\begin{figure}[htb]
\centering \includegraphics[scale=0.6]{figuras/threads_blocks}
\caption{\label{fig:cuda_exec_flow}Organization of kernel execution.}
\end{figure}
...
multiple SM, it is not considered a pure SIMD device.
