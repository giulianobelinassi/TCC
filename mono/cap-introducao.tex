...
Although OpenMP uses simple pragmas to take advantage of multiple CPUs in a shared memory system, GPUs 
...
running code in NVIDIA GPUs. 

CUDA execution is divided into two parts: a host(CPU) and device(GPU) parts. For the device part, CUDA uses 
the concept of kernels, which are functions called from the host to be executed by GPU threads. The 
memory space is also separated between the host and the device, and data should be manually transferred 
between memories in order to allow a device to access them or to retrieve results computed by the GPU.
