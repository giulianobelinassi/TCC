...
$m$ is the number of boundary elements. Algorithm $\ref{Interec}$ sketches this routine.
...
be $b[jj:jj+2]$ and $u[jj:jj+2]$. Let $H_{ij}$ and $G_{ij}$ be the two results from the \texttt{Nonsingd} 
routine from the step $i, j$. Then line 6 of Algorithm $\ref{Interec}$ can be expressed mathematically as: 
...
\begin{equation}
	d_i = \sum_{j=1}^{m} \left(G^{\text{element}}_{ij}b_j \right) - \sum_{j=1}^{m} \left( H^{\text{element}}_{ij}u_j \right)
\end{equation}

Define now two matrices $H \in \Cfield^{(3L)\times (3m)}$ and $G\in \Cfield^{(3L)\times (3m)}$ 
...
Now we only have two assemble two matrices $H$ and $G$ analogously as Algorithm $\ref{ghmatecd_new2}$ without concerning singularity issues 
(just removing lines 10 and 11). After that, just use cuBLAS to multiply $Hu$ and $Gb$, and subtract the resulting vectors. 
