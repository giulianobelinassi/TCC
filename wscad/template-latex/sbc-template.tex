...
This project was supported by CAPES and \textbf{BLABLABLABLABLABLA} (não sei muito bem
o que colocar aqui)
...
\subsection{Parallelization of Ghmatecd}
Ghmatecd works in the following way: It assembles two complex matrices H and G by computing 
smaller $3 \times 3$ matrices returned by Nonsingd and Sigmaec. Sigmaec is very similar to Nonsingd, but it 
is designed to solve the singular part of the dynamic problem. Let $n$ be the number of 
...
is no interdependency between iteractions, so the two nested loop in the algorithm can be computed in parallel, hence, 
both matrices $H$ and $G$ can be built in a parallel fashion. If the number of processors is small, 
then computing that nested loop in parallel is enough because even for small instances of the problem, 
$n \times m$ will be bigger than the number of processors.
By the other hand, GPUs have a huge parallel capability, and the above strategy alone would generate a waste of 
...
\subsection{Parallelization of Ghmatece}
