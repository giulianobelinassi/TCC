...
have experienced strong development due to the presence of increasingly powerful 
...
it naturally satisfies the Sommerfeld radiation condition, also known as geometric damping. Such 
method can be used to numerically model the 3D stationary behavior of wave propagation in 
the soil based on BEM with the objective of creating a computational tool. This tool can assist 
in analysis such as evaluation of vibrations in the soil rising from machine tools operation or 
railway lines, as well as understand the role of soil during earthquakes or even in the design of 
...
routines as shown in Table 1. Since most calls to $\texttt{Nonsingd}$ were from $\texttt{Ghmatecd}$, most of the parallelization effort 
...
For experimenting, there were four data samples as illustrated in Table 2. The 
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.9]{results1.pdf}
...
There are some issues related to the $g$ described in Algorithm 2. Detailed studies are required to determine what is 
