...
Elastodynamic Problems with Graphical Processing Units (GPU)}
...
Such discretization involves in generating two high computational power demanding matrices, 
and this article demonstrates how Graphical Processing Units (GPU) can be used to accelerate this process. 
\end{abstract}
...
CPUs, with more transistors. Recently, as the size of CPU components got smaller, it became
difficult to create such faster sequential CPUs \cite{brock:2006}. As consequence, 
...
there is AMD's 3DNow! \cite{3dnow} and Graphics Processing Units (GPU). 
...
the remaining $H$ and $G$ parts in the CPU. The elapsed time was computed in seconds with the OpenMP library function 
...
multiple kernels. This strategy would allow larger problems to be solved, since if the number of mesh elements is 
large enough the application would crash due to insufficient GPU memory.  Second, the singular and nonsingular 
