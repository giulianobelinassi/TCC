...
\title{Optimizing a Boundary Elements Method implementation for Stationary
Elastodynamic Problems with GPUs}
...
to be implemented into GPUs to explore its massively parallel capabilities. However, primitives 
GPUs were designed to render graphics in real time and all first libraries that allowed GPU 
resources to be used, such as OpenGL, was graphical focused. Such redesigns involved converting 
the original problem into a graphical domain, requiring deep knowledge of the selected graphical library. 
...
formulation of BEM for Stationary Elastodynamic Problems and the meaning of some functions displayed in 
following sections. Next, Section 3 shows how the most computational expensive routine was optimized using 
GPUs. Later, Section 4 discusses how the results were obtained and Section 5 presents and discusses those 
results. Finally, Section 6 briefly discusses possible future works.
...
is completed. With this, a new strategy arises when also computing $\texttt{Nonsingd}$ in parallel. Let $g$ be the number of Gauss quadrature 
points. The pseudocode in Algorithms 2 and 3 pictures this new strategy.

\begin{algorithm}
\caption{Creates $H, G \in \Cfield^{(3m)\times(3n)}$ without Sigmaec part}
\begin{algorithmic}[1]
...
\begin{algorithm}
\caption{Compute Sigmaec part of $H, G \in \Cfield^{(3m)\times(3n)}$}
\begin{algorithmic}[1]
...
2 to 3 will generate the entire $H$ and $G$ without the $\texttt{Sigmaec}$ part. The $\texttt{Ghmatecd\_Sigmaec}$ can be parallelized with 
...
the OpenMP version with 4 CPU threads, and the CUDA and OpenMP together with 4 CPU threads.
...
algorithm are $2.68$ and $106.95$ respectively, the conclusion is that the presented strategy aided with GPUs can be 
...
GPU kernels. Second, there is also no logic to compute both $\texttt{Ghmatecd\_Nonsingd}$ and $\texttt{Ghmatecd\_Sigmaec}$ in 
